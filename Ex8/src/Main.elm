...
init flags location =
    let
        user =
            User.userFromStorage flags.userData

        page =
            hashToPage location.hash |> changeWithPermissions user

        ( productsInitModel, productsInitCmd, _ ) =
            Products.init user
...
update msg model =
    case msg of
        ...
        Navigation (ChangePage page) ->
            let
                updatedPage =
                    changeWithPermissions model.currentUser page
            in
                ( { model | page = updatedPage }, Cmd.none )

        ProductsMsg msg ->
            let
                ( productsModel, cmd, saveData ) =
                    Products.update msg model.products model.currentUser
            in
                ( { model | products = productsModel }, Cmd.batch [ Cmd.map ProductsMsg cmd, trySave saveData ] )
        ...
        Navigation Logout ->
            let
                ( updatedModel, newCmd ) =
                    update (Navigation (ChangePage (Public Landing))) model
            in
                { updatedModel | currentUser = User.Anonymous } ! [ saveUser Nothing ]

        MouseMoved pos ->
            ( { model | mouseX = pos.x, mouseY = pos.y }, Cmd.none )


trySave : Maybe String -> Cmd msg
trySave value =
    case value of
        Nothing ->
            Cmd.none
...
changeWithPermissions : User.CurrentUser -> Page -> Page
...
view : Model -> Html Msg
view model =
    viewLayout model viewCurrentPage Navigation


viewLayout : Model -> (Model -> Html msg) -> (Nav.Msg -> msg) -> Html msg
viewLayout model viewCurrentPage toMsg =
    div []
        [ pageHeader model toMsg
    ...
    in
        div [ class "row" ]
            ...
            , div
                ...
                , style
                    [ ( "position", "fixed" )
                    , ( "left", toString (model.mouseX + 15) ++ "px" )
                    , ( "top", toString (model.mouseY + 15) ++ "px" )
                    ]
                ]
                [ text " " ]
            ]
